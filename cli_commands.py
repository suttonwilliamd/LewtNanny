...
async def cmd_search(args: argparse.Namespace, output: CLIOutput) -> int:
    ...
    try:
        ...
        if item_type in ["all", "attachment"]:
            ...
            results["attachments"] = [
                {"name": a.name, "type": a.attachment_type} for a in attachments[:limit]
            ]

        if item_type in ["all", "resource"]:
            resources = await game_service.search_resources(query, limit)
            results["resources"] = [
                {"name": r.name, "tt_value": str(r.tt_value)} for r in resources[:limit]
            ]

        if item_type in ["all", "blueprint"]:
            blueprints = await game_service.search_blueprints(query)
            results["blueprints"] = [
                {"name": bp.name, "result": bp.result_item} for bp in blueprints[:limit]
            ]
    ...
    try:
        ...
        if item_type in [None, "blueprint"]:
            ...
            if blueprint:
                if output.json_output:
                    materials = [
                        {"name": m.material_name, "quantity": m.quantity}
...
async def cmd_weapons(args: argparse.Namespace, output: CLIOutput) -> int:
    ...
    try:
        ...
        if args.subcommand == "best":
            ...
            output.print_table(
                ["Name", "Type", "DPS", "Economy"], rows, f"Top {limit} Weapons by DPS"
            )
            ...
            output.print_table(
                ["Name", "Type", "Economy", "DPS"],
                rows,
                f"Top {limit} Weapons by Economy",
            )
            ...
            output.print_table(
                ...
                rows,
                f"All Weapons ({len(weapons)} total)",
            )
    ...
    try:
        ...
        if args.subcommand == "type":
            ...
            output.print_table(
                ...
                rows,
                f"Attachments of Type: {attachment_type}",
            )
        else:
            ...
            output.print_table(
                ...
                rows,
                f"All Attachments ({len(attachments)} total)",
            )
...
async def cmd_resources(args: argparse.Namespace, output: CLIOutput) -> int:
    ...
    try:
        ...
        if args.subcommand == "search":
            ...
            output.print_table(
                ["Name", "TT Value"], rows, f"All Resources ({len(resources)} total)"
            )
...
async def cmd_blueprints(args: argparse.Namespace, output: CLIOutput) -> int:
    ...
    try:
        ...
        if args.subcommand == "search":
            ...
            rows = [
                [bp.name, bp.result_item, str(len(bp.materials)) + " materials"]
                for bp in blueprints[:50]
            ]
            output.print_table(
                ["Name", "Result", "Materials"],
                rows,
                f"All Blueprints ({len(blueprints)} total)",
            )
...
async def cmd_sessions(output: CLIOutput) -> int:
    ...
    try:
        ...
        output.print_table(
            ["ID", "Activity", "Cost", "Return", "Start Time"],
            rows,
...
async def cmd_monitor_run(
    ...
    patterns = {
        "damage": re.compile(
            r"\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s+\[System\]\s+\[\]\s+You\s+inflicted\s+([\d.]+)\s+points\s+of\s+damage"
        ),
        "damage_taken": re.compile(
            r"\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s+\[System\]\s+\[\]\s+You\s+took\s+([\d.]+)\s+points\s+of\s+damage"
        ),
        "loot": re.compile(
            r"\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s+\[System\]\s+\[\]\s+You\s+received\s+(.+?)\s+x\s*\(\d+\)\s+Value:\s+([\d.]+)\s+PED"
        ),
        "global": re.compile(
            r"\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s+\[Globals\]\s+\[\]\s+(.+?)\s+killed\s+a\s+creature\s+\((.+?)\)\s+with\s+a\s+value\s+of\s+(\d+)\s+PED!?"
        ),
        "global_team": re.compile(
            r'\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s+\[Globals\]\s+\[\]\s+Team\s+"([^"]+)"\s+killed\s+a\s+creature\s+\((.+?)\)\s+with\s+a\s+value\s+of\s+(\d+)\s+PED!?'
        ),
        "mining": re.compile(
            r"\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s+\[Globals\]\s+\[\]\s+(.+?)\s+found\s+a\s+deposit\s+\((.+?)\)\s+with\s+a\s+value\s+of\s+(\d+)\s+PED!"
        ),
        "craft": re.compile(
            r"\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s+\[Globals\]\s+\[\]\s+(.+?)\s+constructed\s+an\s+item\s+\((.+?)\)\s+worth\s+(\d+)\s+PED!"
        ),
        "hof": re.compile(
            r"\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s+\[Globals\]\s+\[\]\s+A\s+record\s+has\s+been\s+added\s+to\s+the\s+Hall\s+of\s+Fame!"
        ),
    }
                    ...
                    for line in new_content.splitlines():
                        ...
                        if event_type:
                            ...
                            if event_counts["other"] % 50 == 0:
                                print(
                                    f"[{datetime.now().strftime('%H:%M:%S')}] ... (other activity)"
                                )
...
Examples:
  ...
  python cli_commands.py monitor run --loadout zx
    ...
    search_parser.add_argument(
        "--type",
        "-t",
        choices=["weapon", "attachment", "resource", "blueprint", "all"],
        default="all",
    )
    ...
    show_parser.add_argument(
        "--type", "-t", choices=["weapon", "attachment", "resource", "blueprint"]
    )
    ...
    weapons_sub.add_parser("eco", help="Show top weapons by economy").add_argument(
        "--limit", "-l", type=int, default=10
    )
    weapons_sub.add_parser("type", help="List weapons by type").add_argument(
    ...
    attachments_sub.add_parser("type", help="List attachments by type").add_argument(
        "attachment_type", help="Attachment type (e.g., BLP Amp, Scope)"
    )
    ...
    blueprints_sub = blueprints_parser.add_subparsers(
        dest="subcommand", help="Blueprint subcommands"
    )
    ...
    session_sub.add_parser("stats", help="Show session statistics").add_argument(
        "session_id", nargs="?", help="Session ID"
    )
    session_sub.add_parser("delete", help="Delete a session").add_argument(
