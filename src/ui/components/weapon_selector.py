...
from PyQt6.QtWidgets import (
    ...
    QSlider, QProgressBar, QFrame, QSplitter, QInputDialog,
    QMessageBox
)
...
class SimpleValueLabel(QLabel):
    ...
    def setValue(self, new_value: float):
        ...
        display = f"{new_value:.5f}" if new_value < 0.01 else f"{new_value:.4f}" if new_value < 1 else f"{new_value:.2f}"
...
class WeaponSelector(QWidget):
    ...
    async def _load_data(self):
        ...
        try:
            ...
            logger.info(f"Loaded {len(self.attachments)} attachments ({len(self.scopes)} scopes, {len(self.sights)} sights)")
        except Exception as e:
