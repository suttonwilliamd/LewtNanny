...
class TabbedMainWindow(QMainWindow):
    ...
    def create_top_tab_bar(self):
        ...
        for i, tab_name in enumerate(self.TAB_NAMES):
            ...
            btn.clicked.connect(
                lambda checked, name=tab_name, button=btn: self.on_tab_clicked(
                    name, button
                )
            )
    ...
    def create_run_log_table_section(self):
        ...
        self.run_log_table.setSelectionBehavior(
        ...
        self.run_log_table.itemSelectionChanged.connect(
            self._on_run_log_selection_changed
        )
    ...
    def create_item_breakdown_section(self):
        ...
        self.item_breakdown_table.setSelectionBehavior(
            QTableWidget.SelectionBehavior.SelectRows
        )
    ...
    def add_skill_event(self, event_data: Dict[str, Any]):
        ...
        if event_type in ["skill_gain", "skill"]:
            ...
            total_procs = sum(
                int(self.skills_table.item(r, 3).text())
    ...
    def _on_stats_calculated(self, total_cost: float):
        ...
        try:
            ...
            logger.info(
                f"STATS CALCULATED: Updated cost per attack to {self.cost_per_attack:.6f} PED"
            )

            # Update overlay with the synchronized cost
            if self.overlay:
                logger.info(
                    f"STATS CALCULATED: Updating overlay with synchronized cost: {self.cost_per_attack:.6f} PED"
                )
                self.overlay.set_cost_per_attack(self.cost_per_attack)
    ...
    def _on_crafting_cost_added(self, cost: float):
        ...
        try:
            ...
            if (
                ...
                and hasattr(self.overlay.overlay_widget, "_stats")
            ):
                current_cost = float(
                    self.overlay.overlay_widget._stats.get("total_cost", Decimal("0"))
                )
                new_cost = current_cost + abs(cost)  # Cost is positive for spending
                self.overlay.overlay_widget._stats["total_cost"] = Decimal(
                    str(new_cost)
                )
    ...
    def _check_and_update_cost(self):
        ...
        if self.cost_per_attack <= 0:
            ...
            if new_cost > 0:
                ...
                logger.info(
                    f"Cost per attack calculated: {self.cost_per_attack:.6f} PED"
                )
                if self.overlay:
                    self.overlay.set_cost_per_attack(self.cost_per_attack)
                self._update_total_cost_display()
    ...
    def _calculate_cost_per_attack(self) -> float:
        ...
        try:
            ...
            if hasattr(self.config_widget, "ammo_burn_text") and hasattr(
                self.config_widget, "weapon_decay_text"
            ):
                try:
        ...
        try:
            if self.cost_per_attack <= 0:
                ...
                if calculated_cost > 0:
                    ...
                    logger.warning(
                        f"FALLBACK: Calculated cost_per_attack: {self.cost_per_attack:.6f} PED"
                    )
                    if self.overlay:
                        self.overlay.set_cost_per_attack(self.cost_per_attack)
                else:
            ...
            logger.debug(
                f"Updating total cost: shot_cost={shot_cost:.2f} PED (shots={self.total_shots_taken}, cpa={self.cost_per_attack})"
            )
    ...
    def start_session(self):
        ...
        try:
            ...
            if hasattr(self, "combat_widget") and self.combat_widget:
                self.combat_widget.update_session_info(
                    self.current_session_id, self.current_session_start
                )
            ...
            if self.overlay:
                self.overlay.start_session(
                    self.current_session_id, "hunting", self.current_session_start
                )
                self.overlay.set_cost_per_attack(self.cost_per_attack)
    ...
    def stop_session(self):
        ...
        try:
            if self.current_session_id:
                ...
                asyncio.run(
                    self.db_manager.update_session_totals(
    ...
    def toggle_pause_logging(self):
        ...
        # Update chat reader pause state
        if hasattr(self, "chat_reader") and self.chat_reader:
            self.chat_reader.is_paused = self.is_logging_paused
            logger.info(
                f"Chat reader pause state updated: {self.chat_reader.is_paused}"
            )
    ...
    def toggle_streamer_ui(self):
        ...
        if (
            self.overlay.overlay_widget is None
            or not self.overlay.overlay_widget.isVisible()
        ):
            self.overlay.show()
            self.overlay.set_cost_per_attack(self.cost_per_attack)
            if self.current_session_id:
    ...
    def open_donate(self):
        ...
        QDesktopServices.openUrl(
            QUrl("https://www.paypal.com/donate/?hosted_button_id=C8NM596JX8V8E")
        )
    ...
    def _refresh_analysis_data(self):
        ...
        if (
            hasattr(self, "analysis_widget")
            and self.analysis_widget
            and self.db_manager
        ):
            try:
                loop = asyncio.new_event_loop()
    ...
    def _update_analysis_realtime(self):
        ...
        current_session_data = {
            ...
            "start_time": self.current_session_start.strftime("%Y-%m-%d %H:%M:%S")
            if self.current_session_start
            else "",
            "end_time": "",
    ...
    def backup_database(self):
        ...
        try:
            ...
            if db_path.exists():
                backup_path = (
                    data_dir / f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
                )
        ...
        async def _do_update():
            try:
                ...
                self.db_status_label.setText(
                    f"Weapons: {weapon_count} | Sessions: {session_count}"
                )
            except Exception as e:
    ...
    def start_chat_monitoring(self):
        ...
        if hasattr(self, "chat_reader") and self.chat_reader:
            ...
            success = loop.run_until_complete(
                self.chat_reader.start_monitoring(log_path)
            )
            if success:
    ...
    def handle_new_event(self, event_data: Dict[str, Any]):
        ...
        print(
            f"[UI DEBUG] handle_new_event called with: {event_data.get('event_type', 'unknown')}"
        )
    ...
    def _process_event_for_summary(
        ...
        if event_type == "loot":
            ...
            if miss:
                ...
                logger.debug(
                    f"Combat hit: damage={damage}, total_shots={self.total_shots_taken}, cost_per_attack={self.cost_per_attack}"
                )
                self._update_total_cost_display()
            elif dodged:
                self.total_shots_taken += 1
                logger.debug(
                    f"Combat dodge: total_shots={self.total_shots_taken}, cost_per_attack={self.cost_per_attack}"
                )
                self._update_total_cost_display()
    ...
    def _process_loot_event(self, parsed_data: Dict[str, Any]):
        ...
        if not found:
            ...
            self.item_breakdown_table.setItem(
                row, 4, QTableWidgetItem(f"{total_value:.4f}")
            )
            ...
            async def inner():
                ...
                if session_data:
                    ...
                    self.loot_summary_labels["% Return"].setText(
                        f"{return_pct:.1f}%" if total_cost > 0 else "0.0%"
                    )
                ...
                for skill_name, data in skill_totals.items():
                    ...
                    self.skills_table.setItem(row, 1, QTableWidgetItem(skill_name))
                    self.skills_table.setItem(
                        row, 2, QTableWidgetItem(f"{data['total_gain']:.2f}")
                    )
                    self.skills_table.setItem(
                        row, 3, QTableWidgetItem(str(data["procs"]))
                    )
                ...
                for row in range(self.skills_table.rowCount()):
                    ...
                    proc_percent = (
                        (procs / total_procs) * 100 if total_procs > 0 else 100
                    )
                    self.skills_table.setItem(
                        row, 4, QTableWidgetItem(f"{proc_percent:.0f}%")
                    )
