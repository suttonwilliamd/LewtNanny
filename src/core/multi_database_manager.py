...
class MultiDatabaseManager:
    ...
    async def initialize_all(self):
        ...
        # Migrate existing user data from old lewtnanny.db if it exists
        await self._migrate_old_user_data()
    ...
    async def _migrate_all_json_data(self):
        ...
        try:
            ...
            await self._legacy_migrate_json_data()

    async def _migrate_old_user_data(self):
        """Migrate user data from old lewtnanny.db to new user_data.db"""
        old_db_path = self.db_dir / "lewtnanny.db"
        if not old_db_path.exists():
            logger.info("No old lewtnanny.db found, skipping migration")
            return

        # Check if user_data.db already has data
        async with aiosqlite.connect(self.databases["user_data"]) as new_db:
            cursor = await new_db.execute("SELECT COUNT(*) FROM sessions")
            session_count = (await cursor.fetchone())[0]

            if session_count > 0:
                logger.info("user_data.db already contains data, skipping migration")
                return

        logger.info("Migrating user data from old lewtnanny.db...")
        ...
        try:
            async with aiosqlite.connect(old_db_path) as old_db:
                async with aiosqlite.connect(self.databases["user_data"]) as new_db:
                    ...
                    # Migrate sessions (check if table exists first)
                    try:
                        cursor = await old_db.execute("SELECT * FROM sessions")
                        sessions = await cursor.fetchall()
                        for session in sessions:
                            await new_db.execute(
                                """
                                INSERT OR REPLACE INTO sessions 
                                (id, start_time, end_time, activity_type, total_cost, total_return, total_markup)
                                VALUES (?, ?, ?, ?, ?, ?, ?)
                            """,
                                session,
                            )
                    except Exception as e:
                        logger.debug(f"Sessions migration skipped: {e}")

                    # Migrate events
                    try:
                        cursor = await old_db.execute("SELECT * FROM events")
                        events = await cursor.fetchall()
                        for event in events:
                            await new_db.execute(
                                """
                                INSERT OR REPLACE INTO events 
                                (id, timestamp, event_type, activity_type, raw_message, parsed_data, session_id)
                                VALUES (?, ?, ?, ?, ?, ?, ?)
                            """,
                                event,
                            )
                    except Exception as e:
                        logger.debug(f"Events migration skipped: {e}")

                    # Migrate session_loot_items
                    try:
                        cursor = await old_db.execute(
                            "SELECT * FROM session_loot_items"
                        )
                        loot_items = await cursor.fetchall()
                        for item in loot_items:
                            await new_db.execute(
                                """
                                INSERT OR REPLACE INTO session_loot_items 
                                (id, session_id, item_name, quantity, total_value, markup_percent)
                                VALUES (?, ?, ?, ?, ?, ?)
                            """,
                                item,
                            )
                    except Exception as e:
                        logger.debug(f"Session loot items migration skipped: {e}")

                    # Migrate markup_config
                    try:
                        cursor = await old_db.execute("SELECT * FROM markup_config")
                        markups = await cursor.fetchall()
                        for markup in markups:
                            await new_db.execute(
                                """
                                INSERT OR REPLACE INTO markup_config 
                                (item_name, markup_value)
                                VALUES (?, ?)
                            """,
                                markup,
                            )
                    except Exception as e:
                        logger.debug(f"Markup config migration skipped: {e}")

                    # Migrate loadouts
                    try:
                        cursor = await old_db.execute("SELECT * FROM loadouts")
                        loadouts = await cursor.fetchall()
                        for loadout in loadouts:
                            await new_db.execute(
                                """
                                INSERT OR REPLACE INTO loadouts 
                                (id, name, weapon, amplifier, scope, sight_1, sight_2, damage_enh, accuracy_enh, economy_enh, created_at, updated_at)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                            """,
                                loadout,
                            )
                    except Exception as e:
                        logger.debug(f"Loadouts migration skipped: {e}")

                    # Migrate custom_weapons
                    try:
                        cursor = await old_db.execute("SELECT * FROM custom_weapons")
                        custom_weapons = await cursor.fetchall()
                        for custom_weapon in custom_weapons:
                            await new_db.execute(
                                """
                                INSERT OR REPLACE INTO custom_weapons 
                                (id, name, ammo, decay, weapon_type, dps, eco, range_value, damage, reload_time, created_at, updated_at)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                            """,
                                custom_weapon,
                            )
                    except Exception as e:
                        logger.debug(f"Custom weapons migration skipped: {e}")

                    await new_db.commit()

            logger.info(
                f"Migrated {len(sessions)} sessions, {len(events)} events, {len(loot_items)} loot items, {len(markups)} markups, {len(loadouts)} loadouts, and {len(custom_weapons)} custom weapons"
            )

            # Backup old database
            backup_path = old_db_path.with_suffix(".db.backup")
            old_db_path.rename(backup_path)
            logger.info(f"Old lewtnanny.db backed up to {backup_path}")

        except Exception as e:
            logger.error(f"Error migrating old user data: {e}")

    async def _legacy_migrate_json_data(self):
        """Fallback legacy migration method"""
        # Implementation would go here for direct JSON migration
        logger.warning("Legacy JSON migration not implemented")
