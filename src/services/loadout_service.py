...
class LoadoutService:
    ...
    async def get_all_loadouts(self) -> List[WeaponLoadout]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
                "SELECT * FROM loadouts ORDER BY name"
            )
            loadouts = []
    ...
    async def create_custom_weapon(self, weapon: CustomWeapon) -> int:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
                ...
                (name, decay, ammo_burn, dps, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (
                weapon.name,
                str(weapon.decay),
    ...
    async def get_all_custom_weapons(self) -> List[CustomWeapon]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
                "SELECT * FROM custom_weapons ORDER BY name"
            )
            weapons = []
            async for row in cursor:
    ...
    async def get_custom_weapon(self, weapon_id: int) -> Optional[CustomWeapon]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
                "SELECT * FROM custom_weapons WHERE id = ?",
                (weapon_id,)
            )
            row = await cursor.fetchone()
    ...
    async def get_custom_weapon_by_name(self, name: str) -> Optional[CustomWeapon]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute(
                "SELECT * FROM custom_weapons WHERE name = ?",
                (name,)
            )
            row = await cursor.fetchone()
            return CustomWeapon.from_dict(dict(row)) if row else None
    ...
    async def delete_custom_weapon(self, weapon_id: int) -> bool:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute(
                "DELETE FROM custom_weapons WHERE id = ?",
                (weapon_id,)
            )
            await db.commit()
            deleted = cursor.rowcount > 0
