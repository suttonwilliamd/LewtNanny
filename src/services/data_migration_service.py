...
class DataMigrationService:
    ...
    async def migrate_all(self, force: bool = False) -> dict[str, int]:
        ...
        counts = {
            ...
            "blueprint_materials": await self._migrate_blueprint_materials_to_db(
                manager.crafting_db, force
            ),
        }
            ...
            for resource_name, resource_info in data.get("data", {}).items():
                try:
                    ...
                    await db.execute(
                        ...
                        VALUES (?, ?, ?, ?)
                    """,
                        (resource_name, tt_value, decay, updated_dt),
                    )
                    count += 1
                except Exception as e:
            ...
            for blueprint_id, _materials in data.get("data", {}).items():
                try:
                    ...
                    await db.execute(
                        ...
                        VALUES (?, ?, ?, ?)
                    """,
                        (blueprint_id, name, result_item, updated_dt),
                    )
                    count += 1
                except Exception as e:
                ...
                for material in materials:
                    ...
                    try:
                        ...
                        await db.execute(
                            ...
                            VALUES (?, ?, ?)
                        """,
                            (blueprint_id, material_name, int(quantity)),
                        )
                        count += 1
                    except Exception as e:
                        logger.error(
                            f"Error migrating material {material_name} for {blueprint_id}: {e}"
                        )
