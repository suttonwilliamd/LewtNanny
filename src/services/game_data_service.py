...
class GameDataService:
    ...
    async def calculate_blueprint_cost(
        self, blueprint_name: str, resource_values: dict[str, float] | None = None
    ) -> float:
    ...
    def _row_to_attachment(self, row) -> Attachment:
        return Attachment(
            ...
            decay_modifier=(
                Decimal(str(row["decay_modifier"])) if row["decay_modifier"] else Decimal("0")
            ),
            economy_bonus=(
                Decimal(str(row["economy_bonus"])) if row["economy_bonus"] else Decimal("0")
            ),
...
class WeaponCalculator:
    ...
    async def calculate_enhanced_stats(
        ...
        dps = (
            enhanced_damage / base_weapon.reload_time
            if base_weapon.reload_time > 0
            else Decimal("0")
        )
    ...
    async def calculate_session_stats(
        self, weapon_name: str, shots_fired: int
    ) -> dict[str, Any] | None:
