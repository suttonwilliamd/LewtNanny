...
async def generate_sample_data(db_manager: MultiDatabaseManager):
    ...
    for i in range(50):
        ...
        try:
            async with aiosqlite.connect(db_manager.databases["user_data"]) as db:
                await db.execute(
                    """
...
async def clear_sample_data(db_manager: MultiDatabaseManager):
    ...
    try:
        async with aiosqlite.connect(db_manager.databases["user_data"]) as db:
            ...
            await db.execute(
                "DELETE FROM session_loot_items WHERE session_id LIKE 'sample_%'"
            )
...
async def main():
    ...
    parser.add_argument(
        "--clear", action="store_true", help="Clear sample data instead of generating"
    )
