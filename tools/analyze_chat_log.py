...
class ChatLogAnalyzer:
    ...
    def analyze_lines(self):
        ...
        for item in self.lines_captured:
            ...
            if matched:
                self.lines_matched.append({
                    'line': line,
                    'pattern': matched_pattern
                })
            else:
                self.lines_missed.append({
                    'line': line,
    ...
    def print_report(self, category_counts):
        ...
        print(f"Lines matched: {len(self.lines_matched)} ({100*len(self.lines_matched)/max(1,len(self.lines_captured)):.1f}%)")
        print(f"Lines missed: {len(self.lines_missed)} ({100*len(self.lines_missed)/max(1,len(self.lines_captured)):.1f}%)")
